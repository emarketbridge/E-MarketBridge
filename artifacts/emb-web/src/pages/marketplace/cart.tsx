    ...
    <MarketplaceLayout>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground">{t("shoppingCart")}</h1>
            <p className="text-muted-foreground text-sm mt-1">
              {itemCount} {itemCount === 1 ? t("item") : t("itemsPlural")}
            </p>
          </div>
          {cartItems.length > 0 && (
            <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-destructive gap-1" onClick={clearCart} data-testid="button-clear-cart">
        ...
        {cartItems.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-lg font-medium text-foreground">{t("cartEmpty")}</p>
            <p className="text-muted-foreground text-sm mt-1">{t("cartEmptySubtitle")}</p>
            <Link href="/marketplace/products">
              <Button className="mt-6 gap-2" data-testid="button-shop-now">
                <ShoppingCart className="h-4 w-4" />{t("startShopping")}
              </Button>
            </Link>
          </div>
        ) : (
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm font-bold text-primary mt-1">JD {item.price.toFixed(2)} {t("each")}</p>
                          </div>
                          <div className="flex items-center gap-3 flex-shrink-0">
                            ...
                            <p className="text-sm font-bold text-foreground w-20 text-end" data-testid={`text-subtotal-${item.productId}`}>
            ...
            <Card className="border-border bg-muted/30">
              ...
              <CardContent className="space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">{t("subtotal")} ({itemCount} {t("itemsPlural")})</span>
